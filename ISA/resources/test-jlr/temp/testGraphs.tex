% Preamble

\documentclass{amsart}

%\usepackage{amssymb, amscd, amsfonts}
%\usepackage[mathscr]{eucal}
%\usepackage{graphicx}
\usepackage{lscape}
\usepackage{tikz}
\usepackage{scalefnt}
%\usepackage[applemac]{inputenc}
\usepackage[latin1]{inputenc}
%\usepackage[utf8x]{inputenc}
%\usepackage{verbatim}
%\usepackage[export]{adjustbox}
%\usepackage{lipsum}
\usepackage{float}% If comment this, figure moves to Page 2
\usepackage[pgf,outputdir={docgraphs/}]{dot2texi}
%\usepackage[active,tightpage]{preview}
\usetikzlibrary{shapes,automata,arrows}


%% Definitions of proclamation environments, commands, shorthands, et cetera

% Proclamation environments

\newtheorem{theorem}{Theorem}[section]
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{corollary}[theorem]{Corollary}
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem{fact}[theorem]{Fact}

\theoremstyle{definition}
\newtheorem{observation}[theorem]{Observation}
\newtheorem{definition}[theorem]{Definition}
\theoremstyle{remark}
\newtheorem*{algoprob}{Problem}        % To typeset decision problems

\theoremstyle{remark}
\newtheorem*{algoinst}{Instance}       % To typeset decision problems

\theoremstyle{remark}
\newtheorem*{algoquest}{Question}       % To typeset decision problems

\theoremstyle{remark}
\newtheorem*{conjecture}{Conjecture}

\theoremstyle{definition}
\newtheorem{problem}{Problem}

\theoremstyle{remark}
\newtheorem*{question}{Question}

\theoremstyle{definition}
\newtheorem{remark}{Remark}

\theoremstyle{definition}
\newtheorem*{remarks}{Remarks}

\theoremstyle{definition}
\newtheorem{example}{Example}

\theoremstyle{remark}
\newtheorem{examples}{Examples}

\theoremstyle{definition}
\newtheorem{notation}[theorem]{Notation}

\theoremstyle{remark}
\newtheorem*{addnotation}{Additional Notation}

\theoremstyle{remark}
\newtheorem*{runningnotation}{Running Notation}

% Math Commands

\newcommand{\barecoeff}[2]{\gen\frac {} {} {0pt} {} {#1}{#2}}
\newcommand{\wstirl}[2]{\bigl\{\bigl\{\genfrac{}{}{0pt}{}{#1}{#2}\bigr\}\bigr\}}
\newcommand{\sstirl}[2]{ \genfrac {\{} {\}} {0pt} {} {#1}{#2} }
\newcommand{\wbinom}[2]{\bigl(\bigl(\genfrac{}{}{0pt}{}{#1}{#2}\bigr)\bigr)}


% Math Operators

\DeclareMathOperator{\fan}{\rm Fan}
\DeclareMathOperator{\chrom}{\chi} \DeclareMathOperator{\sign}{\rm
signum} \DeclareMathOperator{\partitio}{\rm p}
\DeclareMathOperator{\bbpartitio}{\rm p}
\DeclareMathOperator{\bbcappartitio}{\mathscr{P}}
\DeclareMathOperator{\Sub}{\rm Sub}
\DeclareMathOperator{\Parts}{\mathscr{P}}
\DeclareMathOperator{\MVParts}{\mathscr{MVP}}
\DeclareMathOperator{\Hom}{\textsc Hom}
\DeclareMathOperator{\Var}{\textxsc Var}
\DeclareMathOperator{\F}{\textxsc Form}
\DeclareMathOperator{\Eval}{\textsc Eval}
\DeclareMathOperator{\rank}{\rm rank}
\DeclareMathOperator{\pos}{\rm pos} \DeclareMathOperator{\lin}{\rm
lin} \DeclareMathOperator{\relint}{\rm relint}
\DeclareMathOperator{\con}{\rm conv}
\DeclareMathOperator{\aff}{\rm aff}
\DeclareMathOperator{\supp}{\rm supp}
\DeclareMathOperator{\eff}{\rm eff} \DeclareMathOperator{\st}{\rm
star} \DeclareMathOperator{\cstar}{\rm cstar}
\DeclareMathOperator{\lnk}{\rm link}
\DeclareMathOperator{\rel}{\rm relint}
\DeclareMathOperator{\Dom}{\rm Dom} \DeclareMathOperator{\Cod}{\rm
Cod} \DeclareMathOperator{\Ker}{\rm Ker}
\DeclareMathOperator{\Ran}{\rm Range}
\DeclareMathOperator{\GRan}{\rm GRange}
\DeclareMathOperator{\Dim}{\rm Dim}
\DeclareMathOperator{\vertices}{\rm vert}
\DeclareMathOperator{\Rad}{\mathscr{R}}
\DeclareMathOperator{\Spec}{\bf Spec}
\DeclareMathOperator{\Max}{\bf MaxSpec}
\DeclareMathOperator{\Zero}{\mathbf{Z}}
\DeclareMathOperator{\V}{\mathbb{V}}
\DeclareMathOperator{\I}{\mathbb{I}}
\DeclareMathOperator{\E}{\mathbb{E}}
\DeclareMathOperator{\A}{\mathbb{A}}
\DeclareMathOperator{\Aut}{\rm Aut}
\DeclareMathOperator{\FaceId}{\rm FaceId}
\DeclareMathOperator{\FaceR}{\rm FaceR}
\DeclareMathOperator{\GroupId}{\rm GroupId}
\DeclareMathOperator{\GroupR}{\rm GroupR}
\DeclareMathOperator{\den}{\rm den} \DeclareMathOperator{\lcm}{\rm
lcm}

\DeclareMathOperator{\tree}{\rm Tree}

% Commands

%% Temporary Annotations

\newcommand{\commento}[1]{\marginpar{\footnotesize \flushleft{ #1}}}

\newcommand{\remove}[1]{}

%% Sets of Numbers
\newcommand{\N}{{\mathbb{N}}}
\newcommand{\Prim}{{\mathbb{P}}}
\newcommand{\Z}{{\mathbb{Z}}}
\newcommand{\Q}{{\mathbb{Q}}}
\newcommand{\R}{{\mathbb{R}}}
\newcommand{\C}{{\mathbb{C}}}
\newcommand{\CC}{{\mathscr{C}}}
\newcommand{\lex}{{\overrightarrow{\oplus}}}
\newcommand{\MVN}{{\mathbb{M}\mathbb{V}\mathbb{N}}}
\newcommand{\MVZ}{{\mathbb{M}\mathbb{V}\mathbb{Z}}}
\newcommand{\MVQ}{{\mathbb{M}\mathbb{V}\mathbb{Q}}}
\newcommand{\MVR}{{\mathbb{M}\mathbb{V}\mathbb{R}}}
\newcommand{\MVC}{{\mathbb{M}\mathbb{V}\mathbb{C}}}

%% Frequent Bores
\newcommand{\asc}{\mathrm{asc}}
\newcommand{\des}{\mathrm{des}}

\newcommand{\QQ}{ \mathbb{Q} } % -- Rational numbers
\newcommand{\NN}{\mathbb{N}} % --- Natural numbers
\newcommand{\DD}{\mathcal{D}}
\newcommand{\LL}{\mathcal{L}}

\renewcommand{\SS}{\mathcal{S}}
\newcommand{\vbold}{\mathbf{v}}
\newcommand{\wbold}{\mathbf{w}}
\newcommand{\mss}{{\sf m}}
\newcommand{\M}{{\mathscr{M}}}
\newcommand{\G}{{\mathbf{\Gamma}}}
\newcommand{\Hats}{{\mathbf{H}}}
\newcommand{\lgp}{{$\ell$-group }}
\newcommand{\lgpO}{{$\ell$-group. }}
\newcommand{\lgps}{{$\ell$-groups }}
\newcommand{\lgpsO}{{$\ell$-groups. }}
\newcommand{\Fl}{\mathscr{A}}
\newcommand{\Fln}{\mathscr{A}_n}
\newcommand{\MV}{MV algebra }
\newcommand{\MVO}{MV algebra. }
\newcommand{\MVs}{MV algebras }
\newcommand{\MVsO}{MV algebras. }
\newcommand{\FMV}{\mathscr{M}}
\newcommand{\FMVn}{\mathscr{M}_n}
\newcommand{\Pro}{\textsc{BasProj}}
\newcommand{\In}{\textsc{Input: $\, \,$}}
\newcommand{\Out}{\textsc{Output: $\, \,$}}
\newcommand{\Y}{\textsc{Yes }}
\newcommand{\No}{\textsc{No }}
\newcommand{\pvar}{\vec{\pi}}
\newcommand{\li}{\langle \langle}
\newcommand{\ri}{\rangle \rangle}
\newcommand{\MM}{\mathcal{M}}
\renewcommand{\div}{{\, \, \rm div \,\,}}


%% Funny Names
\newcommand{\Lu}{{\L}u\-ka\-s\-i\-e\-w\-icz }
\newcommand{\Lus}{{\L}u\-ka\-s\-i\-e\-w\-icz's }
\newcommand{\Bez}{Bezout }

% Body
\begin{document}

\title{Graphs Test}

%%% Authors

%\author{O. M. D'Antona}

%%% Date

\date{\today}

%%% Addresses

%\address[O. M. D'Antona]{Dipartimento di Informatica e Comunicazione,
%via Comelico 39/41, I-20135 Milano, Italy}

%\email{dantona@dico.unimi.it}

\keywords{sample.tex}

%\subjclass[2000]{Primary: . Secondary: .}
%%% Abstract

%\begin{abstract}
%Evoluzione? 
%\end{abstract}

\maketitle 

Riportiamo di seguito alcuni possibili famiglie di grafi e relativa tabella degli insiemi indipendenti ottenuti con algoritmo forza bruta.
Questo documento \'e ottenuto eseguendo la classe GraphsTest.java sotto il package test.unimi.isa.graphs

\section{Grafo $P_2^{( 1)}\times P_4^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
; 
\end{tikzpicture}$$

} 

{$$\begin{tabular}{c | r r r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5&6&7\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&2\\ 
$3$&1&6&8&2\\ 
$4$&1&8&18&12&2\\ 
$5$&1&10&32&38&16&2\\ 
$6$&1&12&50&88&66&20&2\\ 
$7$&1&14&72&170&192&102&24&2\\ 
\end{tabular}$$

}

\section{Grafo $P_1^{( 1)}\times P_4^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [right of=1] {1.2}; 
	\node[main node] (3) [right of=2] {1.3}; 
	\node[main node] (4) [right of=3] {1.4}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(4) edge  node[above] {} (3) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5&6\\ \hline 
$0$&1\\ 
$1$&1&1\\ 
$2$&1&2\\ 
$3$&1&3&1\\ 
$4$&1&4&3\\ 
$5$&1&5&6&1\\ 
$6$&1&6&10&4\\ 
$7$&1&7&15&10&1\\ 
$8$&1&8&21&20&5\\ 
$9$&1&9&28&35&15&1\\ 
$10$&1&10&36&56&35&6\\ 
$11$&1&11&45&84&70&21&1\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times P_4^{( 2)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(5) edge [bend right] node[above] {} (1) 
	(7) edge [bend right] node[above] {} (3) 
	(6) edge [bend right] node[above] {} (2) 
	(8) edge [bend right] node[above] {} (4) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&2\\ 
$3$&1&6&6\\ 
$4$&1&8&14&4\\ 
$5$&1&10&26&18&2\\ 
$6$&1&12&42&48&14\\ 
$7$&1&14&62&102&56&6\\ 
\end{tabular}$$

}

\section{Grafo $P_1^{( 1)}\times P_4^{( 2)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [right of=1] {1.2}; 
	\node[main node] (3) [right of=2] {1.3}; 
	\node[main node] (4) [right of=3] {1.4}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(4) edge  node[above] {} (3) 
	(3) edge [bend right] node[above] {} (1) 
	(4) edge [bend right] node[above] {} (2) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&1\\ 
$2$&1&2\\ 
$3$&1&3\\ 
$4$&1&4&1\\ 
$5$&1&5&3\\ 
$6$&1&6&6\\ 
$7$&1&7&10&1\\ 
$8$&1&8&15&4\\ 
$9$&1&9&21&10\\ 
$10$&1&10&28&20&1\\ 
$11$&1&11&36&35&5\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times Z_4^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
(3) edge [] node[above] {} (2)(4) edge [] node[above] {} (1)(5) edge [] node[above] {} (4)(6) edge [] node[above] {} (3)(7) edge [] node[above] {} (6)(8) edge [] node[above] {} (5); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4\\ 
$3$&1&6&4\\ 
$4$&1&8&12\\ 
$5$&1&10&24&8\\ 
$6$&1&12&40&32\\ 
$7$&1&14&60&80&16\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times F_4^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
(4) edge [] node[above] {} (1)(6) edge [] node[above] {} (3)(8) edge [] node[above] {} (5); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&1\\ 
$3$&1&6&6\\ 
$4$&1&8&15&4\\ 
$5$&1&10&28&20&1\\ 
$6$&1&12&45&56&15\\ 
$7$&1&14&66&120&70&6\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times F_5^{( 2)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
	(5) edge [bend right] node[above] {} (1) 
	(7) edge [bend right] node[above] {} (3) 
	(9) edge [bend right] node[above] {} (5) 
	(6) edge [bend right] node[above] {} (2) 
	(8) edge [bend right] node[above] {} (4) 
	(10) edge [bend right] node[above] {} (6) 
(4) edge [] node[above] {} (1)(6) edge [] node[above] {} (3)(8) edge [] node[above] {} (5)(10) edge [] node[above] {} (7); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&1\\ 
$3$&1&6&4\\ 
$4$&1&8&11&2\\ 
$5$&1&10&22&10&1\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times Z_5^{( 2)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
	(5) edge [bend right] node[above] {} (1) 
	(7) edge [bend right] node[above] {} (3) 
	(9) edge [bend right] node[above] {} (5) 
	(6) edge [bend right] node[above] {} (2) 
	(8) edge [bend right] node[above] {} (4) 
	(10) edge [bend right] node[above] {} (6) 
(3) edge [] node[above] {} (2)(4) edge [] node[above] {} (1)(5) edge [] node[above] {} (4)(6) edge [] node[above] {} (3)(7) edge [] node[above] {} (6)(8) edge [] node[above] {} (5)(9) edge [] node[above] {} (8)(10) edge [] node[above] {} (7); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4\\ 
$3$&1&6&2\\ 
$4$&1&8&8\\ 
$5$&1&10&18&2\\ 
$6$&1&12&32&12\\ 
$7$&1&14&50&38&2\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times C_5^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
(1) edge [bend left] node[above] {} (9)(2) edge [bend right] node[above] {} (10); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5&6\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&2\\ 
$3$&1&6&6\\ 
$4$&1&8&16&8&2\\ 
$5$&1&10&30&30&10\\ 
$6$&1&12&48&76&48&12&2\\ 
$7$&1&14&70&154&154&70&14\\ 
\end{tabular}$$

}

\section{Grafo $P_1^{( 1)}\times C_5^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [right of=1] {1.2}; 
	\node[main node] (3) [right of=2] {1.3}; 
	\node[main node] (4) [right of=3] {1.4}; 
	\node[main node] (5) [right of=4] {1.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(4) edge  node[above] {} (3) 
	(5) edge  node[above] {} (4) 
(1) edge [bend left] node[above] {} (5); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5\\ \hline 
$0$&1\\ 
$1$&1&1\\ 
$2$&1&2\\ 
$3$&1&3\\ 
$4$&1&4&2\\ 
$5$&1&5&5\\ 
$6$&1&6&9&2\\ 
$7$&1&7&14&7\\ 
$8$&1&8&20&16&2\\ 
$9$&1&9&27&30&9\\ 
$10$&1&10&35&50&25&2\\ 
$11$&1&11&44&77&55&11\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times CF_5^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
(4) edge [] node[above] {} (1)(6) edge [] node[above] {} (3)(8) edge [] node[above] {} (5)(10) edge [] node[above] {} (7)(1) edge [bend left] node[above] {} (9)(2) edge [bend right] node[above] {} (10); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&1\\ 
$3$&1&6&4\\ 
$4$&1&8&13&2\\ 
$5$&1&10&26&14&1\\ 
$6$&1&12&43&46&9\\ 
$7$&1&14&64&106&50&4\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times CZ_5^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
(3) edge [] node[above] {} (2)(4) edge [] node[above] {} (1)(5) edge [] node[above] {} (4)(6) edge [] node[above] {} (3)(7) edge [] node[above] {} (6)(8) edge [] node[above] {} (5)(9) edge [] node[above] {} (8)(10) edge [] node[above] {} (7)(1) edge [bend left] node[above] {} (9)(2) edge [bend right] node[above] {} (10); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4\\ 
$3$&1&6&2\\ 
$4$&1&8&10\\ 
$5$&1&10&22&4\\ 
$6$&1&12&38&24\\ 
$7$&1&14&58&68&8\\ 
\end{tabular}$$

}

\section{Grafo $P_3^{( 2)}\times CZ_4^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [below of=2] {3.1}; 
	\node[main node] (4) [right of=1] {1.2}; 
	\node[main node] (5) [right of=2] {2.2}; 
	\node[main node] (6) [right of=3] {3.2}; 
	\node[main node] (7) [right of=4] {1.3}; 
	\node[main node] (8) [right of=5] {2.3}; 
	\node[main node] (9) [right of=6] {3.3}; 
	\node[main node] (10) [right of=7] {1.4}; 
	\node[main node] (11) [right of=8] {2.4}; 
	\node[main node] (12) [right of=9] {3.4}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(5) edge  node[above] {} (4) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(9) edge  node[above] {} (8) 
	(11) edge  node[above] {} (10) 
	(12) edge  node[above] {} (11) 
	(3) edge [bend right] node[above] {} (1) 
	(6) edge [bend right] node[above] {} (4) 
	(9) edge [bend right] node[above] {} (7) 
	(12) edge [bend right] node[above] {} (10) 
	(4) edge  node[above] {} (1) 
	(7) edge  node[above] {} (4) 
	(10) edge  node[above] {} (7) 
	(5) edge  node[above] {} (2) 
	(8) edge  node[above] {} (5) 
	(11) edge  node[above] {} (8) 
	(6) edge  node[above] {} (3) 
	(9) edge  node[above] {} (6) 
	(12) edge  node[above] {} (9) 
(4) edge [] node[above] {} (2)(5) edge [] node[above] {} (1)(5) edge [] node[above] {} (3)(6) edge [] node[above] {} (2)(7) edge [] node[above] {} (5)(8) edge [] node[above] {} (4)(8) edge [] node[above] {} (6)(9) edge [] node[above] {} (5)(10) edge [] node[above] {} (8)(11) edge [] node[above] {} (7)(11) edge [] node[above] {} (9)(12) edge [] node[above] {} (8)(1) edge [bend left] node[above] {} (10)(3) edge [bend right] node[above] {} (12); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&3\\ 
$2$&1&6&2\\ 
$3$&1&9&11\\ 
$4$&1&12&31&12&2\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times M_5^{( 1)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
(1) edge [bend left] node[above] {} (10)(2) edge [bend right] node[above] {} (9); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r r r r r r}
$T(n,k)$&$k=0$&1&2&3&4&5&6&7\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4\\ 
$3$&1&6&6&2\\ 
$4$&1&8&16&8\\ 
$5$&1&10&30&30&10&2\\ 
$6$&1&12&48&76&48&12\\ 
$7$&1&14&70&154&154&70&14&2\\ 
\end{tabular}$$

}

\section{Grafo $P_2^{( 1)}\times C_5^{( 3)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [right of=1] {1.2}; 
	\node[main node] (4) [right of=2] {2.2}; 
	\node[main node] (5) [right of=3] {1.3}; 
	\node[main node] (6) [right of=4] {2.3}; 
	\node[main node] (7) [right of=5] {1.4}; 
	\node[main node] (8) [right of=6] {2.4}; 
	\node[main node] (9) [right of=7] {1.5}; 
	\node[main node] (10) [right of=8] {2.5}; 
 \path[every node/.style={font=\sffamily\small}] 	(2) edge  node[above] {} (1) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(3) edge  node[above] {} (1) 
	(5) edge  node[above] {} (3) 
	(7) edge  node[above] {} (5) 
	(9) edge  node[above] {} (7) 
	(4) edge  node[above] {} (2) 
	(6) edge  node[above] {} (4) 
	(8) edge  node[above] {} (6) 
	(10) edge  node[above] {} (8) 
	(1) edge [bend right] node[above] {} (5) 
	(3) edge [bend right] node[above] {} (7) 
	(5) edge [bend right] node[above] {} (9) 
	(7) edge [bend right] node[above] {} (1) 
	(9) edge [bend right] node[above] {} (3) 
	(2) edge [bend right] node[above] {} (6) 
	(4) edge [bend right] node[above] {} (8) 
	(6) edge [bend right] node[above] {} (10) 
	(8) edge [bend right] node[above] {} (2) 
	(10) edge [bend right] node[above] {} (4) 
	(1) edge [bend right] node[above] {} (7) 
	(3) edge [bend right] node[above] {} (9) 
	(5) edge [bend right] node[above] {} (1) 
	(7) edge [bend right] node[above] {} (3) 
	(9) edge [bend right] node[above] {} (5) 
	(2) edge [bend right] node[above] {} (8) 
	(4) edge [bend right] node[above] {} (10) 
	(6) edge [bend right] node[above] {} (2) 
	(8) edge [bend right] node[above] {} (4) 
	(10) edge [bend right] node[above] {} (6) 
(1) edge [bend left] node[above] {} (9)(2) edge [bend right] node[above] {} (10); \end{tikzpicture}$$}

{$$\begin{tabular}{c | r r r}
$T(n,k)$&$k=0$&1&2\\ \hline 
$0$&1\\ 
$1$&1&2\\ 
$2$&1&4&2\\ 
$3$&1&6&6\\ 
$4$&1&8&12\\ 
$5$&1&10&20\\ 
$6$&1&12&30\\ 
$7$&1&14&42\\ 
\end{tabular}$$

}

\section{Grafo $P_3^{( 1)}\times P_4^{( 3)}$ } 

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [below of=2] {3.1}; 
	\node[main node] (4) [right of=1] {1.2}; 
	\node[main node] (5) [right of=2] {2.2}; 
	\node[main node] (6) [right of=3] {3.2}; 
	\node[main node] (7) [right of=4] {1.3}; 
	\node[main node] (8) [right of=5] {2.3}; 
	\node[main node] (9) [right of=6] {3.3}; 
	\node[main node] (10) [right of=7] {1.4}; 
	\node[main node] (11) [right of=8] {2.4}; 
	\node[main node] (12) [right of=9] {3.4}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(5) edge  node[above] {} (4) 
	(6) edge  node[above] {} (5) 
	(8) edge  node[above] {} (7) 
	(9) edge  node[above] {} (8) 
	(11) edge  node[above] {} (10) 
	(12) edge  node[above] {} (11) 
	(4) edge  node[above] {} (1) 
	(7) edge  node[above] {} (4) 
	(10) edge  node[above] {} (7) 
	(5) edge  node[above] {} (2) 
	(8) edge  node[above] {} (5) 
	(11) edge  node[above] {} (8) 
	(6) edge  node[above] {} (3) 
	(9) edge  node[above] {} (6) 
	(12) edge  node[above] {} (9) 
	(7) edge [bend right] node[above] {} (1) 
	(10) edge [bend right] node[above] {} (4) 
	(8) edge [bend right] node[above] {} (2) 
	(11) edge [bend right] node[above] {} (5) 
	(9) edge [bend right] node[above] {} (3) 
	(12) edge [bend right] node[above] {} (6) 
	(10) edge [bend right] node[above] {} (1) 
	(11) edge [bend right] node[above] {} (2) 
	(12) edge [bend right] node[above] {} (3) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r}
$T(n,k)$&$k=0$&1&2&3\\ \hline 
$0$&1\\ 
$1$&1&3&1\\ 
$2$&1&6&8&2\\ 
$3$&1&9&21&12\\ 
$4$&1&12&40&36\\ 
\end{tabular}$$

}

\section{Grafo $P_4^{( 3)}\times P_3^{( 2)}$ }

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [below of=2] {3.1}; 
	\node[main node] (4) [below of=3] {4.1}; 
	\node[main node] (5) [right of=1] {1.2}; 
	\node[main node] (6) [right of=2] {2.2}; 
	\node[main node] (7) [right of=3] {3.2}; 
	\node[main node] (8) [right of=4] {4.2}; 
	\node[main node] (9) [right of=5] {1.3}; 
	\node[main node] (10) [right of=6] {2.3}; 
	\node[main node] (11) [right of=7] {3.3}; 
	\node[main node] (12) [right of=8] {4.3}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(7) edge  node[above] {} (6) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(11) edge  node[above] {} (10) 
	(12) edge  node[above] {} (11) 
	(3) edge [bend right] node[above] {} (1) 
	(4) edge [bend right] node[above] {} (2) 
	(7) edge [bend right] node[above] {} (5) 
	(8) edge [bend right] node[above] {} (6) 
	(11) edge [bend right] node[above] {} (9) 
	(12) edge [bend right] node[above] {} (10) 
	(4) edge [bend right] node[above] {} (1) 
	(8) edge [bend right] node[above] {} (5) 
	(12) edge [bend right] node[above] {} (9) 
	(5) edge  node[above] {} (1) 
	(9) edge  node[above] {} (5) 
	(6) edge  node[above] {} (2) 
	(10) edge  node[above] {} (6) 
	(7) edge  node[above] {} (3) 
	(11) edge  node[above] {} (7) 
	(8) edge  node[above] {} (4) 
	(12) edge  node[above] {} (8) 
	(9) edge [bend right] node[above] {} (1) 
	(10) edge [bend right] node[above] {} (2) 
	(11) edge [bend right] node[above] {} (3) 
	(12) edge [bend right] node[above] {} (4) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r}
$T(n,k)$&$k=0$&1&2&3\\ \hline 
$0$&1\\ 
$1$&1&4\\ 
$2$&1&8&12\\ 
$3$&1&12&36&24\\ 
\end{tabular}$$

}

\section{Grafo $P_4^{( 2)}\times P_3^{( 2)}$ }

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [below of=2] {3.1}; 
	\node[main node] (4) [below of=3] {4.1}; 
	\node[main node] (5) [right of=1] {1.2}; 
	\node[main node] (6) [right of=2] {2.2}; 
	\node[main node] (7) [right of=3] {3.2}; 
	\node[main node] (8) [right of=4] {4.2}; 
	\node[main node] (9) [right of=5] {1.3}; 
	\node[main node] (10) [right of=6] {2.3}; 
	\node[main node] (11) [right of=7] {3.3}; 
	\node[main node] (12) [right of=8] {4.3}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(7) edge  node[above] {} (6) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(11) edge  node[above] {} (10) 
	(12) edge  node[above] {} (11) 
	(3) edge [bend right] node[above] {} (1) 
	(4) edge [bend right] node[above] {} (2) 
	(7) edge [bend right] node[above] {} (5) 
	(8) edge [bend right] node[above] {} (6) 
	(11) edge [bend right] node[above] {} (9) 
	(12) edge [bend right] node[above] {} (10) 
	(5) edge  node[above] {} (1) 
	(9) edge  node[above] {} (5) 
	(6) edge  node[above] {} (2) 
	(10) edge  node[above] {} (6) 
	(7) edge  node[above] {} (3) 
	(11) edge  node[above] {} (7) 
	(8) edge  node[above] {} (4) 
	(12) edge  node[above] {} (8) 
	(9) edge [bend right] node[above] {} (1) 
	(10) edge [bend right] node[above] {} (2) 
	(11) edge [bend right] node[above] {} (3) 
	(12) edge [bend right] node[above] {} (4) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&4&1\\ 
$2$&1&8&14&4\\ 
$3$&1&12&39&36&6\\ 
\end{tabular}$$

}

\section{Grafo $P_4^{( 3)}\times P_4^{( 3)}$ }

{$$\scalefont{0.3}\begin{tikzpicture}
		 [,>=stealth',shorten >=1pt,auto,node distance=1.4cm,thick,main node/.style={circle,draw,font=\sffamily\small}] 
	\node[main node] (1) {1.1}; 
	\node[main node] (2) [below of=1] {2.1}; 
	\node[main node] (3) [below of=2] {3.1}; 
	\node[main node] (4) [below of=3] {4.1}; 
	\node[main node] (5) [right of=1] {1.2}; 
	\node[main node] (6) [right of=2] {2.2}; 
	\node[main node] (7) [right of=3] {3.2}; 
	\node[main node] (8) [right of=4] {4.2}; 
	\node[main node] (9) [right of=5] {1.3}; 
	\node[main node] (10) [right of=6] {2.3}; 
	\node[main node] (11) [right of=7] {3.3}; 
	\node[main node] (12) [right of=8] {4.3}; 
	\node[main node] (13) [right of=9] {1.4}; 
	\node[main node] (14) [right of=10] {2.4}; 
	\node[main node] (15) [right of=11] {3.4}; 
	\node[main node] (16) [right of=12] {4.4}; 

\path[every node/.style={font=\sffamily\small}] 
	(2) edge  node[above] {} (1) 
	(3) edge  node[above] {} (2) 
	(4) edge  node[above] {} (3) 
	(6) edge  node[above] {} (5) 
	(7) edge  node[above] {} (6) 
	(8) edge  node[above] {} (7) 
	(10) edge  node[above] {} (9) 
	(11) edge  node[above] {} (10) 
	(12) edge  node[above] {} (11) 
	(14) edge  node[above] {} (13) 
	(15) edge  node[above] {} (14) 
	(16) edge  node[above] {} (15) 
	(3) edge [bend right] node[above] {} (1) 
	(4) edge [bend right] node[above] {} (2) 
	(7) edge [bend right] node[above] {} (5) 
	(8) edge [bend right] node[above] {} (6) 
	(11) edge [bend right] node[above] {} (9) 
	(12) edge [bend right] node[above] {} (10) 
	(15) edge [bend right] node[above] {} (13) 
	(16) edge [bend right] node[above] {} (14) 
	(4) edge [bend right] node[above] {} (1) 
	(8) edge [bend right] node[above] {} (5) 
	(12) edge [bend right] node[above] {} (9) 
	(16) edge [bend right] node[above] {} (13) 
	(5) edge  node[above] {} (1) 
	(9) edge  node[above] {} (5) 
	(13) edge  node[above] {} (9) 
	(6) edge  node[above] {} (2) 
	(10) edge  node[above] {} (6) 
	(14) edge  node[above] {} (10) 
	(7) edge  node[above] {} (3) 
	(11) edge  node[above] {} (7) 
	(15) edge  node[above] {} (11) 
	(8) edge  node[above] {} (4) 
	(12) edge  node[above] {} (8) 
	(16) edge  node[above] {} (12) 
	(9) edge [bend right] node[above] {} (1) 
	(13) edge [bend right] node[above] {} (5) 
	(10) edge [bend right] node[above] {} (2) 
	(14) edge [bend right] node[above] {} (6) 
	(11) edge [bend right] node[above] {} (3) 
	(15) edge [bend right] node[above] {} (7) 
	(12) edge [bend right] node[above] {} (4) 
	(16) edge [bend right] node[above] {} (8) 
	(13) edge [bend right] node[above] {} (1) 
	(14) edge [bend right] node[above] {} (2) 
	(15) edge [bend right] node[above] {} (3) 
	(16) edge [bend right] node[above] {} (4) 
; 
\end{tikzpicture}$$

}

{$$\begin{tabular}{c | r r r r r}
$T(n,k)$&$k=0$&1&2&3&4\\ \hline 
$0$&1\\ 
$1$&1&4\\ 
$2$&1&8&12\\ 
$3$&1&12&36&24\\ 
$4$&1&16&72&96&24\\ 
\end{tabular}$$

}


\end{document} 