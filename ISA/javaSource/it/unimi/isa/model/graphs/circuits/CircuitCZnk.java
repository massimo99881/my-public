...
public class CircuitCZnk extends CircuitGraphImpl {
	
	public CircuitCZnk(int n, int m, int[] potenzaCamminoVerticale, int[] potenzaCamminoOrizzontale) {
		super(n, m);
		this.n = n;
		this.m = m;
		this.q = this.n - 1;
	...
	public String createForLatex() {
		...
		for(int i=0;i<potenzaCamminoOrizzontale.length;i++){
			tmp2 += PotenzaCamminoCircuito.creaOrizzontale(n, m, potenzaCamminoOrizzontale[i]);
		}
